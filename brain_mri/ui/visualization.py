...
class VisualizationMixin:  # Métodos de visualização de resultados
    ...
    def _resolve_original_path(self, mri_id: str) -> str:  # Resolve caminho relativo do exame original
        # Busca o exame em qualquer orientação disponível (axl/cor/sag), com preferência por axial.
        # Isso permite tolerar sujeitos/exames com imagens em apenas 1–2 planos.
        base_dir = self.dataset_dir
        try:
            if base_dir.name in ("axl", "cor", "sag"):
        ...
        for orient in ("axl", "cor", "sag"):
            for ext in (".nii.gz", ".nii"):  # Tenta extensões comprimida e não comprimida
                p = base_dir / orient / f"{mri_id}_{orient}{ext}"  # Constrói caminho esperado
