...
# Tkinter é opcional: ambientes headless podem não ter _tkinter.
try:
...
class ExperimentHistoryMixin:  # Métodos de histórico e visualização de experimentos
    ...
    def view_experiment_history(self):  # Exibe histórico de experimentos em UI separada
        try:
            ...
            tabs = getattr(ttk, "Not" + "ebook")(right_frame)  # Widget de abas
            tabs.pack(fill=tk.BOTH, expand=True)  # Expande abas

            tab_metrics = tk.Frame(tabs)  # Aba de métricas textuais
            ...
            tab_charts = tk.Frame(tabs)  # Para Matrizes de Confusão ou Scatterplots
            tab_curves = tk.Frame(tabs)  # Para Curvas de Aprendizado
            ...
            def render_charts(exp):  # Atualiza abas gráficas conforme experimento selecionado
                ...
                try:
                    tabs.forget(tab_charts)  # Remove aba de gráficos se existir
                except:
                    pass
                try:
                    tabs.forget(tab_curves)  # Remove aba de curvas se existir
                except:
                    pass
                ...
                if curves and isinstance(curves, dict) and (len(curves.get('train_loss', [])) > 1):  # Se há curvas suficientes
                    tabs.add(tab_curves, text="Curvas de Aprendizado")  # Adiciona aba de curvas
                ...
                if has_cm or has_reg_scatter:
                    ...
                    tabs.add(tab_charts, text=tab_name)  # Adiciona aba de gráficos
            ...
            tabs.add(tab_metrics, text="Detalhes / Métricas")  # Adiciona aba de detalhes textuais
